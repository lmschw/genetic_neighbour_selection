...
add_ranking_by = [False, True, False]
add_own = True
add_random = False
bounds = [-5, 5]
...
if target_order == 1:
    postfix = f"_test_ga_order_distance_n={n}_pi={prob_init}_pm={prob_mut}_pn={prob_intro}_g={num_gens}_pop={population_size}_noise={noise_percentage}_speed={speed}_r={radius}"
elif target_order == 0:
    postfix = f"_test_ga_disorder_distance_n={n}_pi={prob_init}_pm={prob_mut}_pn={prob_intro}_g={num_gens}_pop={population_size}_noise={noise_percentage}_speed={speed}_r={radius}"
else:
    postfix = f"_test_ga_middle_distance_n={n}_pi={prob_init}_pm={prob_mut}_pn={prob_intro}_g={num_gens}_pop={population_size}_noise={noise_percentage}_speed={speed}_r={radius}"
