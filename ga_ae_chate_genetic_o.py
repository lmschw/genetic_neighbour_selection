...
if target_order == 1:
    postfix = f"_test_ga_chate_order_n={n}_pi={prob_init}_pm={prob_mut}_g={num_gens}_pop={population_size}"
elif target_order == 0:
    postfix = f"_test_ga_chate_disorder_n={n}_pi={prob_init}_pm={prob_mut}_g={num_gens}_pop={population_size}"
else:
    postfix = f"_test_ga_chate_middle_n={n}_pi={prob_init}_pm={prob_mut}_g={num_gens}_pop={population_size}"
...
for i in range(num_iters):
    model = GeneticAlgorithmActiveElastic(radius=radius, 
                        tmax=tmax, 
                        density=density, 
                        number_particles=n,
