...
class ActiveElasticRunModel(RunModel):
    ...
    """
    def __init__(self, domain_size, number_particles, radius, c_values, speed=1, a=0.5, b=0.5, alpha=0.1, beta=0.1, spring_constant=0.1, natural_distance=1, sensing_noise=0, actuation_noise=0):
        super().__init__(domain_size=domain_size, radius=radius, noise=sensing_noise, speed=speed, number_particles=number_particles,
                         c_values=c_values, add_ranking_by=[True, False, False], add_own_orientation=True, add_random=False)
    ...
    def get_force(self, positions, orientations):
        ...
        active_elastic_force = np.sum(force_components_c_values, axis=1)

        orientations_force = self.compute_new_orientations(positions, orientations)
        orientations_force = sorient.normalize_orientations(orientations+self.generate_noise())

        return self.a * active_elastic_force + self.b * orientations_force
    ...
    def simulate(self, initialState=(None, None, None), dt=None, tmax=None):
        ...
        for t in range(0, self.tmax, self.dt):
            ...
            forces = self.get_force(positions=positions, orientations=orientations)
