...
from model.active_elastic.run_model_active_elastic_with_orientation_component import ActiveElasticRunModel
...
class GeneticAlgorithmActiveElasticWithOrientationComponent(GeneticAlgorithm):
    def __init__(self, radius, tmax, domain_size=(None, None), density=None, number_particles=None, speed=1, noise_percentage=0, 
                 num_generations=1000, num_iterations_per_individual=10, add_ranking_by=[True, False, False], add_own_orientation=False, add_random=False, 
        ...
        self.c_value_size = number_particles
    ...
    def create_run_model(self, c_values):
        return ActiveElasticRunModel(domain_size=self.domain_size,
                                     ...
                                     c_values=c_values,
                                     speed=self.speed,
                                     sensing_noise=self.noise)
    

class GeneticAlgorithmActiveElasticBasicWithParamsWithOrientationComponent(GeneticAlgorithm):
    def __init__(self, radius, tmax, domain_size=(None, None), density=None, number_particles=None, speed=1, noise_percentage=0, 
        ...
        self.c_value_size = 6 + self.number_particles
        

    def create_run_model(self, c_values):
        alpha = np.absolute(c_values[0])
        beta = np.absolute(c_values[1])
        spring_constant = np.absolute(c_values[2])
        natural_distance = (np.absolute(c_values[3]) / self.bounds[1]) * self.domain_size[0]
        if natural_distance == 0:
            natural_distance = 0.1 # minimal distance e.g. size of particle
        a = np.absolute(c_values[4])
        b = np.absolute(c_values[5])
        c_values = c_values[6:]
        return ActiveElasticRunModel(domain_size=self.domain_size,
                                     number_particles=self.number_particles,
                                     radius=self.radius,
