...
from model.run_model_bins import BinRunModel
...
class GeneticAlgorithmDistribution(GeneticAlgorithm):
    def __init__(self, radius, tmax, domain_size=(None, None), density=None, number_particles=None, speed=1, noise_percentage=0, 
                 ...
                 events=None, bin_bounds=[None], num_bins=None):
        """
        ...
        if any(ele is None for ele in bin_bounds) and (num_bins == None or num_bins == 0):
            raise Exception("Need to provide either the bin bounds or the number of bins")
        elif not any(ele is None for ele in bin_bounds):
            self.bin_bounds = bin_bounds
        else:
            len_bin = (np.absolute(bounds[0]) + np.absolute(bounds[1])) / num_bins
            self.bin_bounds = [np.round(bounds[0] + (len_bin*i), 2) for i in range(num_bins+1)]
        self.c_value_size = (len(self.bin_bounds)-1) ** np.count_nonzero(add_ranking_by) + 1 # num bins per dimension + own influence
    ...
    def create_run_model(self, c_values):
        return BinRunModel(domain_size=self.domain_size,
                                radius=self.radius,
                                noise=self.noise,
                                speed=self.speed,
